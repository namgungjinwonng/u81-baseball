...
th {{ padding: 10px 6px; text-align: center; font-weight: 600; cursor: pointer; white-space: nowrap; user-select: none; }}
...
.team-card-header h3 {{ font-size: 14px; margin-bottom: 2px; }}
.team-card-header .region {{ font-size: 11px; opacity: 0.8; }}
.team-card-body {{ padding: 10px 12px; }}
.team-card-body .info-row {{ display: flex; justify-content: space-between; margin-bottom: 3px; font-size: 12px; }}
...
function showTeamDetail(teamName) {{
    ...
    html += `<div id="modalPosFilter" style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap">`;
    for (const [pos, cnt] of Object.entries(posCounts).sort()) {{
        html += `<div class="pos-filter-btn" data-pos="${{pos}}" onclick="toggleModalPos('${{pos}}')" style="padding:5px 12px;background:#f0f2f5;border-radius:8px;font-size:12px;cursor:pointer;transition:all 0.2s;border:2px solid transparent"><span class="pos-badge pos-${{pos}}">${{pos}}</span> ${{cnt}}명</div>`;
    }}
    html += `</div>`;
    ...
    html += `<div id="modalGradeFilter" style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap">`;
    for (const [grade, cnt] of Object.entries(gradeCounts).sort()) {{
        const gc = grade !== '미지정' ? 'grade-' + grade : '';
        const label = grade !== '미지정' ? grade + '학년' : '미지정';
        html += `<div class="grade-filter-btn" data-grade="${{grade}}" onclick="toggleModalGrade('${{grade}}')" style="padding:5px 12px;background:#f0f2f5;border-radius:8px;font-size:12px;cursor:pointer;transition:all 0.2s;border:2px solid transparent"><span class="grade-badge ${{gc}}" style="display:inline-block;margin-right:4px">${{grade !== '미지정' ? grade : '-'}}</span>${{label}} ${{cnt}}명</div>`;
    }}
    html += `<div onclick="resetModalFilter()" style="padding:5px 12px;background:#666;color:white;border-radius:8px;font-size:12px;cursor:pointer">초기화</div>`;
    html += `</div>`;

    // Filter status
    html += `<div id="modalFilterStatus" style="font-size:12px;color:#888;margin-bottom:10px;display:none"></div>`;
...
function renderModalTable() {{
    ...
    document.querySelectorAll('.pos-filter-btn').forEach(el => {{
        el.style.borderColor = (el.dataset.pos === modalFilterPos) ? '#e94560' : 'transparent';
        el.style.background = (el.dataset.pos === modalFilterPos) ? '#fff0f2' : '#f0f2f5';
    }});
    document.querySelectorAll('.grade-filter-btn').forEach(el => {{
        el.style.borderColor = (el.dataset.grade === modalFilterGrade) ? '#3498db' : 'transparent';
        el.style.background = (el.dataset.grade === modalFilterGrade) ? '#eaf4fd' : '#f0f2f5';
    }});
    ...
    if (modalFilterPos || modalFilterGrade) {{
        const parts = [];
        if (modalFilterPos) parts.push(modalFilterPos);
        if (modalFilterGrade) parts.push(modalFilterGrade !== '미지정' ? modalFilterGrade + '학년' : '미지정');
        statusEl.textContent = `필터: ${{parts.join(' + ')}} — ${{filtered.length}}명`;
        statusEl.style.display = '';
    }} else {{
        statusEl.textContent = `전체: ${{filtered.length}}명`;
        statusEl.style.display = '';
    }}
