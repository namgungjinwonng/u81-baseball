...
# 공식 순위(전적표) 팀명도 동일하게 정규화
if sched.get("official_ranks"):
    for _t, _teams in sched["official_ranks"].items():
        sched["official_ranks"][_t] = [alias.get(n, n) for n in _teams]
...
function compStandings(title){
  ...
  // (동률 처리 기준이 협회 내부 규정이라 자체 계산으로는 재현 불가한 케이스가 있음)
  const off=(scheduleData.official_ranks||{})[title];
  if(off && off.length){
    const oidx={}; off.forEach((n,k)=>{ oidx[n]=k; });
    if(arr.every(t=>oidx[t.name]!=null)){
      arr.sort((a,b)=>oidx[a.name]-oidx[b.name]);
      arr.official=true;
      return arr;
    }
  }
...
function renderStandings(c, box){
  ...
  html+='<div class="stand-note">'+(arr.official
    ?'순위: 대한야구소프트볼협회 공식 순위(전적표) 기준'
    :'순위: 승점(승 2 · 무 1 · 패 0) → 승자승(맞대결) → 전체 실점 → 전체 득점')+'</div>';
